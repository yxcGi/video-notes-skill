...
- **数值类目标帧一律排两个时间点**：讲师"念出数值"的时刻，和它 **+10~20s**。口述永远先于输入，只抽前者会拿到软件默认值或拖动中间值（已三次中招，详见 annotate-style.md 防呆第 1 条）。
...
```bash
# 先激活环境（缺 PIL 时脚本也会自动提示这行）
source /opt/homebrew/Caskroom/miniconda/base/etc/profile.d/conda.sh && conda activate data_processing
...
- **选帧双判据：时刻命中 + 画面独立可读**。三维对象要**三维透视帧**（正对轴线的平面视图看不出操作），理论课避开只有半个公式的过渡帧；讲师视角不佳就沿时间轴前后找旋转/缩放后的帧。
- **30 min 以上的集子预期 2~3 轮定点补抽**：第一批只按章节骨架粗抽，读总览确认时间偏差后再针对缺口密抽——"讲到"与"点到"常差 5~20s，长集里想一批列全只会又多又不准。
...
- **最后 10% 单独再滚一遍**——尾段（删除/改名/保存/总结/答疑）是系统性弱区：处理到这里上下文已满、压缩欲最强，而收尾操作恰是复现必需，已三案并发。
- **链上旧操作不断链**——操作链/推导链上的步骤即使是前文讲过的老操作，也至少给一句显式回指（"复用 3.2 的辅助平面 + 分割主体"），值得就补一图；图链断了读者就认为丢了一步。
